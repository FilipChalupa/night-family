...
Triage feeds dispatch: when triage writes a plan, it also estimates which MCP servers the implementation will need (e.g. the issue links a Linear ticket). Household uses that as a **soft routing preference** — an implement task that needs `linear` is left for a member with `linear` connected, but never starves: if no connected member has it, anyone may take it (degraded, not blocked).
