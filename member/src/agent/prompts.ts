...
export function buildSystemPrompt(opts: {
	...
	const sections: string[] = [
		...
		`- \`bash(command)\` — run a shell command in the workspace (5-minute per-command timeout; commands run under \`/bin/sh\`). Use it for \`ls\`, \`rg\`, tests, builds, formatters, package managers, and \`gh\` read-only commands like \`gh issue view\`, \`gh pr diff\`, \`gh pr view\`, \`gh api ... reactions\`. To search the tree use \`rg\` (installed; fast and skips \`.gitignore\`d / vendored files) — avoid \`grep -R\` over the whole repo, which in a large monorepo can scan \`node_modules\` and hit the 5-minute timeout.`,
